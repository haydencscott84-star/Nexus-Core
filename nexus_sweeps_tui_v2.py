...
class NexusSwingEvents(App):
    ...
    class SweepTable(DataTable):
        ...
        def update_table(self):
            ...
            # --- FIX: ALWAYS REDRAW ---
            # We removed the "signature" check because it was ignoring new trades 
            # if the top "Whale" trade didn't change position.
    ...
    def process_row(self, i, is_history=False):
        try:
            tkr = i.get('ticker')
            if tkr not in TICKERS_TO_SHOW: return
            ...
            if (not is_history and uid not in SEEN_IDS and
                d['ticker'] == 'SPY' and 
