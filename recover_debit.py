...
# RECOVERY PATCH: CLEAN OVERWRITE of populate_debit_chain
...
def apply_recovery():
    try:
        ...
        # We define the CLEAN, CORRECT method.
        # Indentation: 4 spaces for class, 4 for method = 4 total? 
        # No, it's inside a class. 
        # Based on previous `cat`, `def populate...` is at indent 4.
        
        clean_method = """    def populate_debit_chain(self, chain_data, width, target_strike=0.0, ivr=0.0):
        try:
            # DEBUG DUMP (ONE-SHOT) - UNCONDITIONAL TO TMP
            try: 
                with open('/tmp/debit_debug.txt', 'a') as f: 
                    f.write("DEBUG_START\\n")
                    if chain_data:
                        f.write(f"DATA_SAMPLE: {chain_data[0]}\\n")
                    else:
                        f.write("NO_DATA\\n")
            except: pass
...
"""
        ...
        if re.search(regex, content, re.DOTALL):
            ...
            print("SUCCESS: Method Overwritten Cleanly.")
