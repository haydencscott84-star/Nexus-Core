...
class TestApp(App):
    ...
    async def load_history_background(self):
        ...
        try:
            ...
            self.query_one("#lbl", Label).update(f"Data Loaded. Starting Persistence Thread...")
            
            # [FIX] Run Persistence in Background Executor!
            daily_stats = await loop.run_in_executor(None, analyze_persistence, full_df)
            
            self.query_one("#lbl", Label).update(f"Done: {len(daily_stats)} rows")
