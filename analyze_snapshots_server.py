...
class StrategicHUD(App):
    ...
    def fetch_and_save_snapshots(self):
        ...
        try:
            ...
            # [CRITICAL FIX] Check for recent data to prevent CRASH LOOP LEAK
            # If we have a SPY snapshot from the last 15 minutes, SKIP FETCH.
            try:
                found_recent = False
                check_path = "snapshots_spy"
                if os.path.exists(check_path):
                    for f in os.listdir(check_path):
                        if f.endswith(".csv"):
                            # Format: spy_snapshot_YYYY-MM-DD_HHMMSS.csv
                            # Extract HHMMSS
                            try:
                                parts = f.split('_')
                                if len(parts) >= 3:
                                    ts_str = parts[-1].replace(".csv", "")
                                    # We need date too? Assume filename has date.
                                    # actually datestr is parts[-2]
                                    date_str = parts[-2]
                                    full_str = f"{date_str}_{ts_str}"
                                    file_dt = datetime.strptime(full_str, "%Y-%m-%d_%H%M%S")
                                    
                                    # Check age
                                    age = datetime.now() - file_dt
                ...
                if found_recent:
                    self.write_log("🛑 SKIPPING FETCH: Recent Data Found (<15m).")
                    return # EXIT EARLY
                    
            except Exception as e:
                self.write_log(f"⚠️ Freshness Check Error: {e}")
