...
def update_google_sheet_ledger():
    ...
    # [FIX] Filter out any accidentally injected SPY options (which have strikes ~660)
    # We only want to analyze SPX index options (strikes > 3000) per the user's request.
    ...
    if os.path.exists("market_state.json"):
        try:
            with open("market_state.json", "r") as f:
                m_state = json.load(f)
                ts_spot = float(m_state.get("market_structure", {}).get("SPX", {}).get("price", 0))
        except Exception as e: print(f"⚠️ Could not load TS_Nexus SPX spot price: {e}")
    ...
    try:
        ...
        updates = []
        updates.append([f"TOP 15 MOST TRADED STRIKES ({s_date} to {e_date})", "", "", "", "", "", "", "TS_Nexus SPX Spot $\\rightarrow$", f"${ts_spot:.2f}"])
        ...
        print("Successfully published to 5-Day Flow Ledger tab!")
        
        # [NEW] DUAL-WRITE TO SUPABASE
        print("☁️ Syncing SPX 5-Day Flow Ledger to Supabase...")
        ...
        asyncio.run(upload_json_to_supabase("nexus_profile", payload, id_field="id", id_value="spx_flow_ledger"))
        print("✅ SYNCHRONIZED SPX LEDGER WITH SUPABASE")
