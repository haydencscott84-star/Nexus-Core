...
class ReversionProApp(App):
    ...
    def on_mount(self) -> None:
        ...
        self.load_state()
        self.run_analysis()
        # Active Measuring Loop (5s) to update Current Extension without heavy backtest
        self.set_interval(5.0, self.update_live_extension)
    ...
    async def async_analysis(self):
        ...
        try:
            ...
            bars_needed = (252 * YEARS_BACK) + SMA_PERIOD + 50
            # FIX: get_historical_data is synchronous, do not await!
            df_bars = ts.get_historical_data(TICKER, bars_needed, "Daily")
            
            if df_bars:
                df = pd.DataFrame(df_bars)
                # Ensure numeric cols
                df['Close'] = pd.to_numeric(df['Close'])
                df['Date'] = pd.to_datetime(df['TimeStamp'] if 'TimeStamp' in df.columns else df['Timestamp'])
    ...
    def update_live_extension(self):
        """Lightweight loop to update Current Extension label only."""
        # Need rudimentary check if we have data
        if not hasattr(self, 'last_df') or self.last_df is None or self.last_df.empty: return
        
        try:
             # Run in thread to allow UI updates
             self.run_worker(self.async_live_update(), exclusive=False)
        except: pass

    async def async_live_update(self):
        try:
            ts = TradeStationManager(TS_CLIENT_ID, TS_CLIENT_SECRET, TS_ACCOUNT_ID)
            quote = ts.get_quote_snapshot(TICKER)
            
            if quote and ('Last' in quote or 'Ask' in quote):
                price = float(quote.get('Last') or quote.get('Ask'))
                
                # Recalculate Extension using known SMA
                # SMA doesn't change intraday enough to matter for 'Daily' 200SMA usually
                # But if we want perfection, we'd append this bar.
                # For simplicity/speed: Use last calculated SMA from backtest.
                
                # Recalc last SMA just in case
                if 'SMA_200' in self.last_df.columns:
                    sma = self.last_df['SMA_200'].iloc[-1]
                    if sma > 0:
                        curr_ext = ((price - sma) / sma) * 100
                        
                        # Update Label safely
                        def _upd():
                            self.query_one("#live_ext_lbl").update(f"📊 LIVE EXTENSION: {curr_ext:.2f}%")
                            
                            # Optional: Update highlighting? 
                            # That requires re-rendering table row styles.
                            # For now, just the label confirms 'Active Measuring'.
