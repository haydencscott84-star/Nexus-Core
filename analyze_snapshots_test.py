...
class StrategicHUD(App):
    ...
    #heatmap-container { height: 1fr; border-bottom: solid $secondary; }
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="heatmap-container"):
            yield DataTable(id="dt_heatmap")
            with TabbedContent():
                with TabPane("Whale Traps", id="tab_traps"):
    ...
    async def on_mount(self):
        ...
        dt_spx.add_columns(
            ...
            Text("BE", style="bold gold"), 
            Text("SPOT", style="bold gold"), 
            Text("STATUS", style="bold gold"), 
            Text("NET Δ", style="bold gold"),
            Text("GAMMA", style="bold gold"),
            Text("VEGA", style="bold gold"),
            Text("THETA", style="bold gold")
        )
        ...
        dt_spy.add_columns(
            ...
            Text("BE", style="bold cyan"), 
            Text("SPOT", style="bold cyan"), 
            Text("STATUS", style="bold cyan"), 
            Text("NET Δ", style="bold cyan"),
            Text("GAMMA", style="bold cyan"),
            Text("VEGA", style="bold cyan"),
            Text("THETA", style="bold cyan")
        )
        ...
        await self.refresh_analysis()
        self.set_interval(900.0, self.refresh_analysis) # User Req: 15 min refresh
    ...
    def build_heatmap(self):
        dt = self.query_one("#dt_heatmap", DataTable)
        dt.clear(columns=True)
        
        # Pivot: Index=Strike, Columns=Date, Values=OI Delta
        # Filter for SPY/SPX main ticker
        # Use both SPY and SPX (normalized) to ensure we capture all days
        target_stats = self.daily_stats[self.daily_stats['ticker'].isin(["SPY", "SPX"])].copy()
        if target_stats.empty: return

        # Normalize SPX strikes to SPY levels for the heatmap
        target_stats['plot_strike'] = target_stats.apply(lambda x: x['strike']/10 if x['ticker'] == "SPX" else x['strike'], axis=1)
        
        # Pivot
        pivot = target_stats.pivot_table(index='plot_strike', columns='date', values='oi_delta', aggfunc='sum').fillna(0)
        pivot.sort_index(ascending=False, inplace=True) # High strikes top
        
        # Add Columns
        dt.add_column("STRIKE", width=10)
        date_cols = sorted(pivot.columns)
        for d in date_cols:
            dt.add_column(d.strftime("%m-%d"))
            
        # Add Rows
        # Limit to reasonable range around spot? Or top OI?
        # Let's show top 20 active strikes by total volume
        # AND Filter by +/- 10% of Spot Price (if known)
        
        # Estimate Spot from last underlying price if available
        last_spot = 600.0 # Fallback
        if 'underlying_price' in self.current_df.columns:
            last_spot = self.current_df['underlying_price'].iloc[-1]
            
        # Scaling Fix: If SPX and spot < 1000, scale up. If SPY and spot > 1000, scale down.
        # Since we normalized everything to SPY levels (plot_strike), we expect spot ~600
        if last_spot > 2000: last_spot /= 10 # Normalize spot if it's SPX
        
        top_strikes = target_stats.groupby('plot_strike')['vol'].sum().nlargest(50).index
        
        # Filter Range
        lower = last_spot * 0.90
        upper = last_spot * 1.10
        filtered_strikes = [s for s in top_strikes if lower <= s <= upper]
        
        # If filtering removed everything (e.g. bad spot price), revert to top 20
        if not filtered_strikes:
            filtered_strikes = top_strikes[:20]
        else:
            filtered_strikes = filtered_strikes[:20] # Cap at 20
            
        pivot = pivot.loc[pivot.index.intersection(filtered_strikes)]
        pivot.sort_index(ascending=False, inplace=True)
        
        for strike, row in pivot.iterrows():
            cells = [Text(f"{strike:.0f}", style="bold white")]
            for d in date_cols:
                val = row[d]
                # Color Logic
                style = "dim white"
                if val > 5000: style = "bold green"
                elif val > 1000: style = "green"
                elif val < -5000: style = "bold red"
                elif val < -1000: style = "red"
                
                # Ghost/Fortress markers could be added here if we had per-cell flags easily accessible
                # For now, just OI Delta intensity
                cells.append(Text(f"{val:+.0f}", style=style))
            dt.add_row(*cells)
    ...
    async def build_kill_box(self, live_spx=0.0, live_spy=0.0):
        ...
        calls['type'] = 'CALL' # [FIX] Add explicit type column
        ...
        puts['type'] = 'PUT' # [FIX] Add explicit type column
        ...
        # FALLBACK LOGIC: If no traps, show Top OI (Support/Resistance)
        fallback_mode = False
        if trapped.empty:
            fallback_mode = True
            trapped = merged.copy() # Use all
        
        # Sorting
        # Sort by absolute OI Delta (Total Exposure) regardless of type
        trapped['abs_exposure'] = trapped['oi_delta'].abs()
        spx_traps = trapped[trapped['ticker'] == 'SPX'].sort_values(by=['abs_exposure'], ascending=False).head(10)
        spy_traps = trapped[trapped['ticker'] == 'SPY'].sort_values(by=['abs_exposure'], ascending=False).head(20)
        
        # --- ENRICH WITH GREEKS ---
        self.log_msg("Enriching Kill Box with Greeks...")
        ...
        self.log_msg(f"Found Traps -> SPX: {len(spx_traps)}, SPY: {len(spy_traps)}")

        # [ASYNC FIX] run in thread to avoid blocking UI
        loop = asyncio.get_event_loop()
        
        # SPX ENRICHMENT disabled for stability test
        # try:
        #    spx_traps = await asyncio.wait_for(loop.run_in_executor(None, enrich_traps_with_greeks, spx_traps), timeout=5.0)
        # except Exception as e: self.log_msg(f"⚠️ SPX Timeout: {e}")

        # SPY ENRICHMENT disabled for stability test
        # try:
        #     spy_traps = await asyncio.wait_for(loop.run_in_executor(None, enrich_traps_with_greeks, spy_traps), timeout=5.0)
        # except Exception as e: self.log_msg(f"⚠️ SPY Timeout: {e}")
        
        self.log_msg("⚠️ Greeks Enrichment Disabled (Stability Mode)")
        # --------------------------
        
        # [FIX] Get Real SPX Price from Data if available (BEFORE POPULATING TABLES)
        spx_rows = active_df[active_df['ticker'] == 'SPX']
        if not spx_rows.empty and 'underlying_price' in spx_rows.columns:
            # Use max to avoid zeros/bad ticks (assuming market is open or was open)
            spx_cand = spx_rows['underlying_price'].max()
            if spx_cand > 2000: SPX_PRICE = spx_cand
        
        # --- POPULATE SPX TABLE ---
        for _, row in spx_traps.iterrows():
            spot_disp = SPX_PRICE
            ...
            status = row['status']
            if "BULLS" in status:
                trap_style = "bold red"      # Danger: Longs puking
                fuel_label = "TRAPPED BULLS"
            elif "BEARS" in status:
                trap_style = "bold green"    # Opportunity: Shorts covering
                fuel_label = "TRAPPED BEARS"
            else:
                trap_style = "bold blue"
                fuel_label = "RESISTANCE" if row['type'] == 'CALL' else "SUPPORT"

            # OI Delta Formatting
            oid_val = row['oi_delta']
            if abs(oid_val) < 1:
                oid_str = "-"
                oid_style = "dim white"
            else:
                oid_str = fmt_oi_delta(oid_val)
                oid_style = f"bold {('green' if oid_val>0 else 'red')}"
            
            # DTE Styling
            dte_val = row['dte']
            if dte_val <= 1: dte_style = "bold red"
            elif dte_val > 30: dte_style = "bold blue"
            else: dte_style = "white"
            ...
            dt_spx.add_row(
                Text(row['ticker'], style=tkr_style),
                Text(f"{row['strike']:.0f}", style=val_style),
                Text(f"{dte_val:.0f}d", style=dte_style),
                Text(f"${row['breakeven']:.2f}", style=val_style),
                Text(f"${spot_disp:.2f}", style=val_style),
                Text(fuel_label, style=trap_style),
                Text(oid_str, style=oid_style),
                Text(f"{gamma:.2f}", style="dim white"),
                Text(f"{vega:.2f}", style="dim white"),
                Text(f"{theta:.2f}", style="dim white")
            )
            
        # --- POPULATE SPY TABLE ---
        for _, row in spy_traps.iterrows():
            spot_disp = SPY_PRICE
        ...
        for _, row in spx_traps.iterrows():
            ...
            dt_spx.add_row(
                Text(row['ticker'], style=tkr_style),
                Text(f"{row['strike']:.0f}", style=val_style),
                Text(f"{row['dte']:.0f}d", style=dte_style),
                Text(f"${row['breakeven']:.2f}", style=val_style),
                Text(f"${spot_disp:.2f}", style=val_style),
                Text(fuel_label, style=trap_style),
                Text(oid_str, style=oid_style),
                Text(f"{gamma:.4f}", style="dim white"), # [FIX] 4 decimals for Deep ITM visibility
                Text(f"{vega:.4f}", style="dim white"),
                Text(f"{theta:.4f}", style="dim white")
            )
