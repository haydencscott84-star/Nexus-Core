...
class StrategicHUD(App):
    ...
    async def on_mount(self):
        ...
        dt_spx.add_columns(
            ...
            Text("Γ", style="bold gold"),    # Restored Gamma
            Text("ν", style="bold gold"),    # Restored Vega
            Text("θ", style="bold gold"),    # Restored Theta
            Text("RENT", style="bold gold"), 
            Text("DAYS LEFT", style="bold gold")
        )
        ...
        dt_spy.add_columns(
            ...
            Text("Γ", style="bold cyan"),    # Restored Gamma
            Text("ν", style="bold cyan"),    # Restored Vega
            Text("θ", style="bold cyan"),    # Restored Theta
            Text("RENT", style="bold cyan"),
            Text("DAYS LEFT", style="bold cyan")
        )
    ...
    async def build_kill_box(self, live_spx=0.0, live_spy=0.0):
        ...
        # --- ENRICH WITH GREEKS ---
        # (Using imported module if available, mocked otherwise)
        # We already have Mean Greeks from aggregation, so we might not need external enrichment 
        # unless 'enrich_traps_with_greeks' gets Live Greeks vs Snapshot Greeks.
        # For now, we trust the snapshot aggregation or existing flow.
        ...
        for dt, data in [(dt_spx, spx_traps), (dt_spy, spy_traps)]:
            for _, row in data.iterrows():
                ...
                dt.add_row(
                    ...
                    Text(rent_str, style=r_style),
                    Text(day_str, style=day_style)
                )
        try:
