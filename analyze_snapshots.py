...
class StrategicHUD(App):
    ...
    async def load_history_background(self):
        try:
            ...
            try:
                ...
                if upload_json_to_supabase:
                    try:
                        asyncio.create_task(upload_json_to_supabase("nexus_profile", history_state, id_field="id", id_value="nexus_history"))
                    except Exception as e:
                        self.log_msg(f"Supabase History Upload Error: {e}")
            except Exception as e:
    ...
    async def build_kill_box(self, live_spx=0.0, live_spy=0.0):
        ...
        try:
             ...
             # Re-inject Distance Percentage for Frontend rendering cleanly
             def _sync_dist(r):
                 s = SPX_PRICE if r['ticker'] == 'SPX' else SPY_PRICE
                 if s > 0: return round((abs(s - r['strike']) / s) * 100, 2)
                 return 0.0
             if not combined_bridge_df.empty:
                 combined_bridge_df['dist_pct'] = combined_bridge_df.apply(_sync_dist, axis=1)
             ...
             if upload_json_to_supabase:
                 try:
                     asyncio.create_task(upload_json_to_supabase("nexus_profile", quant_payload, id_field="id", id_value="nexus_quant"))
                 except Exception as e:
                     self.log_msg(f"Supabase Quant Upload Error: {e}")
            ...
            if not df.empty:
                ...
                if upload_json_to_supabase:
                    try:
                        loop = asyncio.get_event_loop()
                        if loop.is_running():
                            loop.create_task(upload_json_to_supabase("nexus_profile", history_state, id_field="id", id_value="nexus_history"))
                        else:
                            loop.run_until_complete(upload_json_to_supabase("nexus_profile", history_state, id_field="id", id_value="nexus_history"))
                    except RuntimeError:
                        asyncio.run(upload_json_to_supabase("nexus_profile", history_state, id_field="id", id_value="nexus_history"))
                    except Exception as e:
                        print(f"Supabase Headless Upload Error: {e}")
            else:
