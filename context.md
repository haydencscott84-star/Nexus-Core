...
| **2** | **WATCHTOWER** | `watchtower_engine.py` | **Core**: Stop-Loss & Anomaly Monitor |
| **3** | **SWEEPS_V3** | `nexus_sweeps_v3.py` | **Data**: Whale Hunter (High Threshold) |
| **4** | **GREEKS** | `nexus_greeks.py` | **Math**: Portfolio Delta/Gamma Engine |
...
| **8** | **ANALYZE_SNAPSHOTS** | `analyze_snapshots.py` | **Logic**: Trap Analysis & **RVOL HUD** |
| **9** | **ALERTS** | `alert_manager.py` | **Logic**: Alert Manager |
| **10** | **BRIDGE** | `market_bridge_v2.py` | **System**: Metric Unification & Persistence |
...
| **19** | **NOTIFICATIONS** | `nexus_notifications.py` | **System**: ZMQ->Discord Router |
| **20** | **GUARDIAN** | `nexus_guardian.py` | **System**: File Liveness Monitor |
...
| **25** | **OI_BOOK** | `nexus_oi_book.py` | **Data**: OI Order Book (Net Change) |
| **26** | **SWEEPS_TAPE** | `nexus_sweeps_tui_v1.py` | **Data**: Tape Reader (0-3 DTE) |
...
### I. Feb 04 Upgrades (SPY Magnet & Viewer)

#### 1. SPY "Magnet" Implementation (Viewer Dash)
- **Feature**: Replicated the "Magnet" logic from SPX Profiler for SPY.
- **Logic**: **Raw Volume Point of Control**.
  - Formula: Strike with highest `Call Volume + Put Volume` (Matches SPX exactly).
  - *Correction*: Initially deployed "Notional Volume" logic, but corrected to "Raw Volume" to ensure math identity with Sheets.
- **UI**: Added `| Magnet: $XXX` to Viewer Dashboard header.
  - **State Handling**: Displays `Magnet: WAIT...` if value is 0 or loading.
- **Data Flow**: `volume_poc_strike` is now exported in `nexus_spy_profile.json` as the `magnet` key.

#### 2. Viewer Stability
- **Crash Fix**: Wrapped `HeaderBox.update_header` in robust `try/except` block to prevent silent crashes during data loading.
- **Environment**: Verified `launch_cockpit.sh` uses valid `TERM=xterm-256color` and `LANG=C.UTF-8` for Textual apps.
...
### O. System Optimization & Resource Governance (Feb 1, 2026)

**Objective**: Eliminate resource wastage during off-hours and prevent memory leaks from destabilizing the server, without losing data integrity during market hours.

#### 1. "Hiber-Nation" Protocol (Market Schedule)
- **Problem**: 24/7 scanning consumed significant CPU/RAM/API credits even when markets were closed.
- **Solution**: Implemented a centralized **Market Schedule** in `nexus_config.py`.
    - **Hours**: 04:00 - 20:00 ET (Mon-Fri) + Sunday Futures (18:00+).
    - **Behavior**:
        - `uw_nexus.py`: Disconnects WebSocket and sleeps 5m blocks during off-hours.
        - `nexus_sweeps_v3.py`: Pauses API polling and ZMQ streaming.
        - **Watchdog Awareness**: The Watchdog now respects this schedule and will **NOT** restart non-essential services (like Sweeps) if they stop during off-hours.

#### 2. Watchdog V2 ("The Sheriff")
- **Upgrade**: `system_watchdog.py` evolved from a simple checks-alive script to a **Resource Governor**.
- **Features**:
    - **Metric**: Monitors `RSS` (RAM) and `%CPU` via `ps` command integration (No extra dependencies).
    - **Governance Rules**:
        - **RAM Limits**: `UW_NEXUS` (2.5GB), `SWEEPS_V3` (1.5GB), `HISTORY` (1.2GB), Default (800MB).
        - **Action**: **OOM Kill & Restart**. If a service breaches its limit, the Sheriff restarts the specific Tmux window to flush memory leaks.
    - **Self-Healing**: `uw_nexus.py` now includes an internal memory check to `sys.exit(0)` voluntarily if approaching the limit, triggering a clean Watchdog restart.

#### 3. Service-Level Optimizations
- **UW Nexus (Window 13)**: Added 2.5GB Memory Ceiling and Hibernate Logic.
- **Sweeps V3 (Window 19)**: Added Hibernate Logic and fixed a loop-crash bug caused by legacy variable referencing (`NameError`).

### P. Profiler & Reporting Upgrades (Feb 04, 2026)

#### 1. SPX Profiler Optimization (Dynamic Polling)
- **Objective**: Increase responsiveness during market hours while conserving API credits.
- **Implementation**:
    - **Market Hours (09:30 - 16:00 ET)**: 3-Minute Loop (Flow + GEX Sync).
    - **Off-Hours**: 5-Minute Loop.
    - **Result**: Flow updates are now 40% faster during active trading.

#### 2. Weighted Sentiment Engine
- **Problem**: Broad "Neutral" filters caused the Sentiment Score to flatline at 0 during chop.
- **Solution**: Implemented a **Weighted Scoring System**:
    - **Premium Weight**: `<10k` (0.1x), `10k-50k` (0.5x), `50k-200k` (1.0x), `200k-500k` (2.0x), `>500k` (5.0x).
    - **Conviction Boost**: "Strong" trades get **1.5x** multiplier.
    - **Relaxed Filters**: Now captures "Lean" trades (Urgency > +/- 0.05).
    - **Display**: Score is now a float (e.g., `+12.5`) for granular tracking.

#### 3. Trading Day Extension
- **Feature**: Extended the "Trading Day" until **06:00 AM ET** the following morning.
- **Purpose**: Prevents the Daily Sentiment Score from resetting to 0 at midnight, preserving context for late-night analysis.

#### 4. Google Sheets Reporting
- **Weekend Restriction**: `nexus_sheets_bridge.py` now strictly **Skips Updates** on Sat/Sun to prevent data clutter.
- **Force Index 2-EMA**: Added smooth FI metric (Window 7 -> Sheets) for cleaner trend analysis.
- **Hourly RVOL**: Switched from Real-time to Hourly RVOL for stable volume context.

#### 5. Service Restoration
- **MTF Nexus**: Stabilized via virtual environment encapsulation.
- **GEX Worker**: Fixed context window to calculate premiums for +/- 300 strikes.
- **Viewer Dashboard**: Increased market-hours polling rate to **3 Minutes** (was 5m).



### Q. Dual Sweeps & OI Logic (Feb 16, 2026)

#### 1. Dual Sweeps Architecture (Barbell Strategy)
- **Objective**: Solve the "Signal vs Noise" trade-off by running two dedicated engines.
- **Window 3 ("The Signal")**: `nexus_sweeps_v3.py`.
    - **Filter**: SPX > $500k, SPY > $50k.
    - **Role**: Alerts, Whale Hunting, Market Structure.
- **Window 26 ("The Tape")**: `nexus_sweeps_tui_v1.py`.
    - **Filter**: SPX > $50k, SPY > $25k.
    - **Role**: Flow Context, Retail Participation, "Reading the River".
    - **UI**: Restricted to **0-3 DTE Only** to minimize cognitive load.

#### 2. OI Order Book (Window 25)
- **Engine**: `nexus_oi_book.py`.
- **Logic**: Visualizes Open Interest as a "Depth Chart" (Level 2).
- **Features**:
    - **Net Change**: Highlights Fresh Positioning (Day-over-Day).
    - **Buckets**: 0-3 DTE (Gamma), 4-30 DTE (Swing), 31+ DTE (Structural).

### R. Dashboard Metric Expansion & Textual App Restarts (Feb 27, 2026)

#### 1. Trader Dashboard Metrics (Theta)
- **Feature**: Expanded the `trader_dashboard_v3.py` (Window 14) to display combined `GAMMA | THETA` metrics inline, combining dual values into a single header cell.
- **Data Source**: Reused existing Greek payload generated by `nexus_greeks.py`, surfacing the pre-calculated `theta` value into the Textual UI.

#### 2. Textual App "Zombie" Prevention (SIGINT Interception)
- **Problem**: Deploy scripts using `tmux send-keys C-c` failed to stop Textual applications (`trader_dashboard_v3.py`), resulting in "Zombie" processes that ignored code updates for weeks.
- **Root Cause**: The `Textual` framework intercepts and handles the standard `SIGINT` (Ctrl+C) command natively, preventing the process from terminating conventionally in a tmux scripting environment.
- **Protocol**: When updating code for Textual-based UI applications, **ALWAYS** use `pkill -f [script_name]` or fully kill and recreate the tmux window (`tmux respawn-window -k`) to force the old process to clear before deployment.

### S. Architectural Cleanups & Data Racing (March 4, 2026)

#### 1. Unusual Whales API Decoupling (UI Simplification)
- **Objective**: Reduce system dependency on the Unusual Whales API for high-frequency tick data that is not critical to the core structural analysis.
- **Actions**:
    - **SPX Profiler & Snapshot Analyzer**: The historical "Sweeps" and "Flow Tape" logic pipelines were entirely stripped out. The bottom UI panels displaying "0-3 DTE" and "3+ DTE" historical premium flows were disabled.
    - **Fallback Logic**: Open Interest (OI) bars in the UI now default to a consistent static rendering format (Green for Calls, Red for Puts) instead of attempting to calculate intraday flow changes, eliminating the need for high-frequency historical data streaming.

#### 2. 0DTE Focus (Dominant Expiration Logic)
- **Problem**: The `analyze_snapshots.py` script frequently skipped over the active 0DTE (Same-Day) options chain to focus on Friday or Monthly expirations when calculating "Dominant Expiration".
- **Fix**: The logic was refactored to explicitly prioritize the **Current Date** (0DTE/1DTE) when calculating Wall Metrics and Gamma Flip points, ensuring the Market Structure analysis perfectly reflects the immediate, intraday trading landscape.

#### 3. Delta Premium Race Condition ($141B Spike)
- **Problem**: The Google Sheets integration outputted nonsensical Delta Premium values (e.g., $141.6B) for the Call Wall.
- **Root Cause**: A race condition where three separate scripts were simultaneously overwriting the `nexus_walls_context.json` file.
    - `gex_worker_nexus.py` calculated true **Net Dealer Delta Notional** ($\textasciitilde$\$1.5B).
    - `spx_profiler_nexus.py` mistakenly wrote **Total Gamma Exposure** into the delta file.
    - `analyze_snapshots.py` calculated **Gross Notional Delta** (the absolute combined size of all options), ballooning the value to massive proportions.
- **Resolution**: Stripped the redundant output commands from `spx_profiler_nexus.py` and `analyze_snapshots.py`. `gex_worker_nexus.py` is now the exclusive *Single Source of Truth* for generating the Wall Context file.

### T. Delta Premium Accumulation & Flow Tracking (March 6, 2026)

#### 1. Intraday Accumulation State (JSON)
- **Problem**: Dashboards traditionally calculated "Net Premium" as an absolute snapshot (Total Volume * Spot Price), leading to bloated numbers that didn't reflect true intraday direction.
- **Solution**: Shifted from Snapshots to **Accumulation**. Both `viewer_dash_nexus.py` and `spx_profiler_nexus.py` now run a `track_accumulated_premium` parser which compares live volume against previous cycle volume stored in local tracking files (`viewer_premium_state.json` and `spx_premium_state.json`).
- **Result**: The "Net Flow (Bull/Bear)" metric now displays the true, incrementally summed premium generated over the course of the exact trading session.

#### 2. Delta Formatter Tuning
- **Fix**: The "Net Delta" string was outputting into the Billions (-$42.0B) because it was mistakenly multiplying contract volume by the Underlying Spot Price (Notional Dollar Delta).
- **Update**: The logic was stripped to calculate pure **Delta Shares** (`Volume * Contract Delta * 100`).

#### 3. Support/Resistance Strike Selectors (Near-Term Filter)
- **Fix**: Removed the strict `Volume < OI` filter, and injected a strict Expiration Filter (`<= 7 DTE`). The Dashboard TUIs now accurately snap to Support and Resistance strikes within the active, immediate weekly trading range.

### U. Visual Formatting & UI Framework Stability (March 16, 2026)

#### 1. Flow Ledger Sentiment Tracking & Decimal Precision
- **Challenge**: The Google Sheets `update_ledger_sheet.py` (SPX) and `spy_ledger_sheet.py` (SPY) were misclassifying all "Option Open Interest Covered/Closed" (Negative Δ OI) trades as "Other", and Google Sheets was rounding their fractional volume ratios (Vol/OI) into invisible/empty blocks.
- **Resolution (Sentiment)**: Expanded the internal `sentiment_str` logic matrices to explicitly define all 4 states of positioning:
  - **Added**: `Bullish (Short Put Buildup)`, `Bearish (Long Put Buildup)`.
  - **Covered (Negative Δ OI)**: `Bearish (Short Put Cover)`, `Bullish (Long Put Close)`.
  - Conditional Formatting (Google Sheets API) was rewritten from strict text matches to `TEXT_CONTAINS` wildcard regex searches to paint all "Bullish" rows Green and "Bearish" rows Red regardless of their specific suffix.
- **Resolution (Vol/OI Formatting)**: Replaced raw python `float` math with explicit two-decimal f-string text injection (`f"{value:.2f}"`) for `Vol/OI` and `RVOL`. This completely bypassed Google Sheets' internal rounding engines, forcing it to render exact precision decimals natively on the screen (e.g. `0.33` instead of empty).

#### 2. Analyze Snapshots Multi-Billion Layout 
- **Challenge**: Giant institutional Index flow caused Notional Dollar strings to overflow beyond UI columns in `analyze_snapshots.py` (e.g. rendering 12.5 Billion as `12552.1M`).
- **Resolution**: Restructured the visual calculation loops for `fmt_notional` and `fmt_oi_delta`. System now detects absolute integers `>= 1,000,000,000`, divides them cleanly by `1e9`, and appends the `B` suffix. Result: `12552.1M` compresses neatly into `12.5B`.

#### 3. Trader Dashboard Greek Zeroing (Race Condition)
- **Challenge**: `trader_dashboard_v3.py` header metrics (Delta, Gamma, Theta, Vega) were strictly returning `+0.00` despite `nexus_greeks.py` successfully querying ORATS and dumping complex portfolio math to the local state file.
- **Root Cause**: The Textual UI initialization loop was crashing behind-the-scenes with `AttributeError: Metric object has no attribute renderable`. The internal Greek thread was attempting to `update()` the textual elements with streaming market derivatives *before* the application had completely mounted the grid layout to the memory space.
- **Resolution**: Initialized the raw root class (`super().__init__("")`) explicitly during the inheritance phase of the `Metric` wrapper. Deployed a strict `asyncio.sleep(2.0)` startup lag to the `monitor_greeks` function to ensure the TUI is visually active and attached before the data extraction triggers.

### V. Streamlit Supabase Synchronization (March 18, 2026)

#### 1. Sheets Bridge & Ledger Sync
- **Problem**: The Streamlit dashboard "Raw Options Flow" and "Market Regime" tables stopped updating, appearing permanently stale, despite Google Sheets receiving live data correctly.
- **Root Cause**: The VPS was running an outdated version of `nexus_sheets_bridge.py` lacking the `upload_json_to_supabase` logic required to sync data to the Streamlit database.
- **Resolution**: Overwrote the outdated script with the fully-featured `remote_nexus_sheets_bridge.py` logic natively on the server. Dual-write functions now execute flawlessly.
- **Protocol**: When editing Sheets integration, ALWAYS ensure the server version includes the Supabase upload blocks at the end of the script before deploying. The canonical script is `nexus_sheets_bridge.py` (Window 23).
